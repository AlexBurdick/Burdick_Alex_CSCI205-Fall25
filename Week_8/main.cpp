...
void testClosed() {
    ofstream closedPut("closedPut.txt"); // Files for graphing
    ofstream closedGet("closedGet.txt");
    ofstream closedRem("closedRem.txt");
    int putTester = 0;                   // Counters for efficiency testing
    int getTester = 0;
    int remTester = 0;
    ...
    cout << "Testing put-\n";
    for (int i = 0; i < numStrings; ++i) {
        ht.put(testStrings[i], testFloats[i], putTester);
        closedPut << ht.size() << " " << putTester << "\n";
    }
    cout << "\nHash Table after insertion\n" << endl;
    ht.print();
    ...
    for (int i = 0; i < 7; ++i) {
        try {
            int val = ht.get(testStrings[i], getTester);
            closedPut << ht.size() << " " << getTester << "\n";
    ...
    for (int i = 0; i < 3; i++) {
        bool removed = ht.remove(keysToRemove[i], remTester);
        closedRem << ht.size() << " " << remTester << "\n";
